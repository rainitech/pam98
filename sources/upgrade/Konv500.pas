...
const
 ...
 _cis_bu:TMyUp=(ope:'';nam:'cis_bu';new:'_cis_bu';adr:'data';typ:'1110000';siz:13);
 _hp1:TMyUp=(ope:'';nam:'hp1';new:'_hp1';adr:'data';typ:'1010000';siz:5);
 _hp2:TMyUp=(ope:'';nam:'hp2';new:'_hp2';adr:'data';typ:'1010000';siz:9);
 _tlac_par:TMyUp=(ope:'';nam:'tlac_par';new:'_tlacpar';adr:'data';typ:'1000000';siz:7);
 _inf:TMyUp=(ope:'';nam:'inf';new:'_inf';adr:'data';typ:'1000000';siz:85);
 _vyp:TMyUp=(ope:'';nam:'vyp';new:'_vyp';adr:'data';typ:'1111100';siz:247);
 _vypnp:TMyUp=(ope:'';nam:'vypnp';new:'_vypnp';adr:'data';typ:'1111100';siz:30);
 _zam:TMyUp=(ope:'';nam:'zam';new:'_zam';adr:'data';typ:'1111111';siz:100);
 _zrv:TMyUp=(ope:'';nam:'zrv';new:'_zrv';adr:'data';typ:'1110000';siz:16);
...
begin
  ...
  Delete(iban, 1, 4);
  for a := 'A' to 'Z' do iban := StrReplaceAll(iban, a, IntToStr(Ord(a) - 55));
  v := 1;
  l := 9;
  rest := 0;
  alpha := '';
...
function MakeIBAN(iban: string): string;
var i,j:integer;
begin
  for i:=9 downto 0 do for j:=9 downto 0 do begin
    iban[3]:=char(ord('0')+i);
    iban[4]:=char(ord('0')+j);
    if CheckIban(iban) then begin Result:=iban; exit; end;
  end;
  Result:='';
end;

function BBAN2IBAN(predcislo_u: string; cislo_u:string; banka_u: string): string;
begin
  if predcislo_u='' then predcislo_u := '000000';
  if cislo_u='' then begin Result:=''; exit; end;
  if banka_u='' then begin Result:=''; exit; end;
  Result := MakeIBAN('SK00'
          + LPAD(banka_u,4,'0')
          + LPAD(predcislo_u,6,'0')
          + LPAD(cislo_u,10,'0')
  );
end;
...
begin
  ...
  upgrade_table(_hp1,chyba); if chyba<>'' then exit;
  upgrade_table(_hp2,chyba); if chyba<>'' then exit;
  upgrade_table(_tlac_par,chyba); if chyba<>'' then exit;
  upgrade_table(_vypnp,chyba); if chyba<>'' then exit;
...
{ nastav hodnotu pre parameter 36 }
    ...
    if T1.FieldByName('ID').Value=36 then begin
      T1.Edit;
      T1.FieldByName('Hodnota').Value:=19458.00;
      T1.Post;
    end;
    T1.Filter:='';
    T1.Filtered:=False;
{ nastav hodnotu pre parameter 28 }
    ...
    if T1.FieldByName('ID').Value=28 then begin
      T1.Edit;
      T1.FieldByName('Hodnota').Value:=21.41;
      T1.Post;
    end;
    T1.Filter:='';
    T1.Filtered:=False;
  ...
  if (iRok=DEF_ROK_DO) then begin
   ...
   { kalendar pre rok 2014 }
    ...
    T1.FieldByName('MAJ').Value := 22;
    T1.FieldByName('JUN').Value := 21;
    T1.FieldByName('JUL').Value := 23;
    T1.FieldByName('AUG').Value := 21;
    T1.FieldByName('SEP').Value := 22;
    T1.FieldByName('OKT').Value := 23;
    T1.FieldByName('NOV').Value := 20;
    T1.FieldByName('DEC').Value := 23;
    T1.FieldByName('JAN1').Value := 2;
    ...
    T1.FieldByName('JUL1').Value := 0;
    T1.FieldByName('AUG1').Value := 1;
    T1.FieldByName('SEP1').Value := 2;
    T1.FieldByName('OKT1').Value := 0;
    T1.FieldByName('NOV1').Value := 1;
    T1.FieldByName('DEC1').Value := 3;
    {--}
    T1.FieldByName('F_NEMMAX').Value := 26.4658;
    T1.FieldByName('F_NEMMIN').Value := 0;
    {--}
    T1.Post;
    T1.Close;
  end else
