...
const
  ...
  MenoCopyright='Copyright 1996-2008 Rainitech, s.r.o';
...
SMyAppInvalidObdobie4Demo=
  ...
  'bolo urèené na vıplatné mesiace január a jún.';
SMyAppInvalidValue=
  'Nesprávna hodnota.';
SMyAppInvalid2Value=
...
SMyAppNaozajSpustitGfkGarancne=
  'Nepouívajte túto funkciu bez písomného súhlasu od spoloènosti RAINITECH !!!. eláte si spusti nastavenie garanèného poistenia pre všetkıch zamestnancov ?';
...
SMyVypOtazkaUplatnitZrazkovuDan=
 'Zrákovou daòou je moné zdani príjem zamestnanca do vıšky %s Sk mesaène v prípade zamestnanca, ktorı zamestnávate¾a nepoiadal o uplatòovanie nezdanite¾nıch èiastok a daòového bonusu. Naozaj si eláte uplatni zrákovú daò ?';
...
var MyAppPath:string[128];
    ...
    MyDataskPth:string[128];  {dáta z aktuálneho roku}
    MyDataDir:string[8];
...
var MyTextDoplatky:string[64];
var MyParser:array[1..50] of string;
var dobre_cu:boolean;
...
uses
  ...
  ComCtrls,Registry,Progress{,About,RxCalc},DbTables;
...
begin
  ...
  FormMyOtazka.StaticText1.Text:=S;
...
begin
  ...
  if Frac(X)<>0 then begin
    if X>0 then X:=X+0.000000001 else X:=X-0.00000001;
  end;  
  if S=0 then begin
    if Frac(X)<>0 then begin
      if X<0
...
begin
  ...
  try c1:=StrToInt(ww[1].hod); except oUlica:=TRIM(pom1); supCislo:=''; orCislo:=''; exit; end;
  if Trim(ww[2].hod)<>'/' then begin oUlica:=TRIM(Copy(pom1,1,ww[1].zaciatok-1)); orCislo:=IntToStr(c1); supCislo:=''; exit; end;
  try c3:=StrToInt(ww[3].hod); except oUlica:=TRIM(Copy(pom1,1,ww[1].zaciatok-1)); orCislo:=IntToStr(c1); supCislo:=''; exit; end;
