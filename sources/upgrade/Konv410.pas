...
const
 ...
 _rzp:TMyUp=(ope:'';nam:'rzp';new:'_rzp';adr:'data';typ:'1110000';siz:44);
...
begin
 ...
 if X.nam = 'rzp' then begin
   T2.FieldByName('Zdravotnici_zvysenie').Value := 0;
   T2.FieldByName('Premia_zaklad').Value := 0;
   T2.FieldByName('Premia_mesiacov').Value := 0;
   T2.FieldByName('Premia_suma').Value := 0;
   T2.FieldByName('Premia_narok').Value := false;
   T2.FieldByName('Premia_manualne').Value := false;
 end
end;
...
begin
  ...
  if (iRok=DEF_ROK_DO) then begin
   ...
   { kalendar pre rok 2010 }
    T1.Open;
    T1.Edit;
    T1.FieldByName('JAN').Value := 21;
    T1.FieldByName('FEB').Value := 20;
    T1.FieldByName('MAR').Value := 23;
    T1.FieldByName('APR').Value := 22;
    T1.FieldByName('MAJ').Value := 21;
    T1.FieldByName('JUN').Value := 22;
    T1.FieldByName('JUL').Value := 22;
    T1.FieldByName('AUG').Value := 22;
    T1.FieldByName('SEP').Value := 22;
    T1.FieldByName('OKT').Value := 21;
    ...
    T1.FieldByName('MAJ1').Value := 0;
    T1.FieldByName('JUN1').Value := 0;
    T1.FieldByName('JUL1').Value := 1;
    T1.FieldByName('AUG1').Value := 0;
    T1.FieldByName('SEP1').Value := 2;
    T1.FieldByName('OKT1').Value := 0;
    T1.FieldByName('NOV1').Value := 2;
    T1.FieldByName('DEC1').Value := 1;
    T1.Post;
    T1.Close;
  end else
  begin
