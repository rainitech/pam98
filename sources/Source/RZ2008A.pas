...
type
  TReportRZ_2008 = class(TQuickRep)
    DetailBand1: TQRBand;
    QRLabel32: TQRLabel;
    QRLabel36: TQRLabel;
    QRLabel38: TQRLabel;
    ...
    procedure ReportRZ_2008BeforePrint(Sender: TQuickRep;
...
var
  ReportRZ_2008: TReportRZ_2008;

procedure RunReportRZ_2008(const MyReportParam: TMyReportParam);
procedure RunReport_NEZISK1_2008(const MyReportParam: TMyReportParam);
procedure RunReport_NEZISK2_2008(const MyReportParam: TMyReportParam);
...
procedure TReportRZ_2008.ShowReportStart;
begin
  first_preview:=True;
...
procedure TReportRZ_2008.ShowReportEnd;
begin
  QINF_.Close;
  Q0_.Close;
end;

procedure TReportRZ_2008.Q0_CalcFields(DataSet: TDataSet);
...
procedure RunReportRZ_2008(const MyReportParam: TMyReportParam);
begin with ReportRZ_2008 do begin
  X:=MyReportParam;
  Screen.Cursor:=crHourGlass;
  try
  try
    ReportRZ_2008:=TReportRZ_2008.Create(Application);
    if X.DisableShape then QRDisableShape(ReportRZ_2008);
    if X.DisableLabel then QRDisableLabel(ReportRZ_2008);
    Screen.Cursor:=crDefault;
    ReportRZ_2008.ShowReportStart;
    if Q0_.Active AND (Q0_.RecordCount>0) then ReportRZ_2008.Preview;
    ReportRZ_2008.ShowReportEnd;
...
begin with ReportRZ_2008 do begin
  ...
  try
    ...
    Screen.Cursor:=crDefault;
    ReportRZ_2008.ShowReportStart;
    if ReportRZ_2008.Q0_.Active AND (ReportRZ_2008.Q0_.RecordCount=0) then
    begin
      MyError(Format(SMyRzpNeexistuje2,[MyRok(X.AktObdobie)-1,MyRok(X.AktObdobie)]));
      exit;
    end;
    X.rtf_sablona := MyFrmPath + 'Vyhlasenie_2%_2007.rtf';
    X.rtf_subor :=
...
begin with ReportRZ_2008 do begin
  ...
  try
    ...
    X.rtf_sablona := MyFrmPath + 'Potvrd_dane_nezisk_sektor_2008.rtf';
    X.rtf_subor :=
    ...
    begin
      ...
      Rtf_SetKeyWord('$RE4',Format('%0.2f',[Q0_EUR_NEDOPL_DAN.Value]),0,' ');
...
procedure TReportRZ_2008.QINF_CalcFields(DataSet: TDataSet);
begin
...
procedure TReportRZ_2008.ReportRZ_2008BeforePrint(Sender: TQuickRep;
  var PrintReport: Boolean);
begin
