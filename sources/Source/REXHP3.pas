...
procedure RunReportHP3(const MyReportParam: TMyReportParam);
...
procedure TReportHP3.ShowReport;
...
procedure TReportHP3.PageHeaderBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
begin
  ...
  Screen.Cursor:=crHourGlass;
  try
    ReportHP3:=TReportHP3.Create(Application);
    if X.DisableShape then QRDisableShape(ReportHP3);
    if X.DisableLabel then QRDisableLabel(ReportHP3);
    Screen.Cursor:=crDefault;
    if X.HPID>0
      then ReportHP3.ShowReport
