...
var
  ReportDVP: TReportDVP;

procedure RunReportDVP(const MyReportParam: TMyReportParam);
...
procedure TReportDVP.ShowReport;
begin
  Q0_.Close;
...
procedure TReportDVP.Q0_CalcFields(DataSet: TDataSet);
begin
...
begin
  ...
  try
  try
    ReportDVP:=TReportDVP.Create(Application);
    if X.DisableShape then QRDisableShape(ReportDVP);
    if X.DisableLabel then QRDisableLabel(ReportDVP);
    Screen.Cursor:=crDefault;
    ReportDVP.ShowReport;
