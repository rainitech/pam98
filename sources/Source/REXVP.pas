...
procedure RunReportVP(const MyReportParam: TMyReportParam);
...
procedure TReportVP.ShowReport;
var tl2na1: boolean;
begin
...
procedure TReportVP.Q1_CalcFields(DataSet: TDataSet);
  var Z,S,V : double;
...
begin
  ...
  try
  try
    ReportVP:=TReportVP.Create(Application);
    if X.DisableShape then QRDisableShape(ReportVP);
    if X.DisableLabel then QRDisableLabel(ReportVP);
