...
type
  TReportVD2009A = class(TQuickRep)
    Q0_: TQuery;
    QRBand1: TQRBand;
    QRLabel1: TQRLabel;
    QRShape1: TQRShape;
    ...
    procedure ReportVD2009ABeforePrint(Sender: TQuickRep;
...
var
  ReportVD2009A: TReportVD2009A;

procedure RunReportVD2009A(const MyReportParam: TMyReportParam);
procedure RunReportVD2009A_PDF(const MyReportParam: TMyReportParam);
...
procedure TReportVD2009A.ShowReportStart;
...
begin
  ...
  R1:=preddavok;
  rr1.Caption := Format('%0.2f',[R1]);
  rr6.Caption := Format('%0.2f',[R6]);
  rr0.Caption := Format('%0.2f',[d_ciastzd]);
  Q0_.Close;
  Q0_.SQL.Clear;
  ...
  R7:=R5+R6;
  R10:=R7;
  rr2.Caption := Format('%0.2f',[R2-R3]);
  {rr3.Caption := CurrToStr(R3); rr3 bolo zlucene do rr2}
  rr4.Caption := Format('%0.2f',[R5]);
  celkovy_preplatok:=R10-R11;
  {rr5.Caption := CurrToStr(R5);}
  rr6.Caption := Format('%0.2f',[R6]);
  rr7.Caption := Format('%0.2f',[R7]);
  {rr10.Caption := CurrToStr(R10);}
  rr11.Caption := Format('%0.2f',[R11]);
  rr12.Caption := Format('%0.2f',[R12]);
  R13 := bonus_kryty_zrazkou;
  rr13.Caption := Format('%0.2f',[bonus_kryty_zrazkou]);
  rr14.Caption := Format('%0.2f',[R12-R13]);
  rr15.Caption := Format('%0.2f',[skut_odvedena_dan]);
  rra.Caption  := Format('%0.2f',[bonus]);
  rrb.Caption  := Format('%0.2f',[bonus_kryty_preddavkom+bonus_kryty_zrazkou]);
  rrc.Caption  := Format('%0.2f',[bonus_vypl_z_prostr_firmy]);
  rrd.Caption  := Format('%0.2f',[bonus_vypl_z_prostr_firmy]);
  {rr21.Caption  := CurrToStr(R10);}
  {rr22.Caption := CurrToStr(R11);}
  if celkovy_preplatok<0 then begin
    rr23.Caption := Format('%0.2f',[-celkovy_preplatok]);
    rr24.Caption := Format('%0.2f',[0.0]);
  end
  else begin
    rr23.Caption := Format('%0.2f',[0.0]);
    rr24.Caption := Format('%0.2f',[celkovy_preplatok]);
  end;
...
procedure TReportVD2009A.ShowReportEnd;
begin
end;
...
begin
  ...
  try
  try
    ReportVD2009A:=TReportVD2009A.Create(Application);
    if X.DisableShape then QRDisableShape(ReportVD2009A);
    if X.DisableLabel then QRDisableLabel(ReportVD2009A);
    Screen.Cursor:=crDefault;
    ReportVD2009A.ShowReportStart;
    ReportVD2009A.Preview;
    ReportVD2009A.ShowReportEnd;
...
procedure RunReportVD2009A_PDF(const MyReportParam: TMyReportParam);
var A1,A2:string;
begin with ReportVD2009A,DM do begin
  X:=MyReportParam;
  Screen.Cursor:=crHourGlass;
  try
  try
...
procedure TReportVD2009A.ReportVD2009ABeforePrint(Sender: TQuickRep;
  var PrintReport: Boolean);
begin
