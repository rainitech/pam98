...
//            ISO 639 language constants added.
...
uses
  WideStrUtils, UnicodeUtils, ABNFUtils, SysUtils, Classes;
...
begin
  ...
  for I := 1 to Length(S) do begin
    if IsAbnfALPHAChar(S[I]) then begin
      TagStart := False;
      Inc(LetterCount);
      if LetterCount > 8 then begin Result := False; Exit; end;
    end else if IsAbnfDIGITChar(S[I]) then begin
      if PrimaryTag then begin Result := False; Exit; end;
      TagStart := False;
      Inc(LetterCount);
      if LetterCount > 8 then begin Result := False; Exit; end;
    end else if S[I] = '-' then begin
      if TagStart then begin Result := False; Exit; end;
      PrimaryTag := False;
      TagStart := True;
      LetterCount := 0;
    end;
...
function IsSubLanguage(const Sublanguage, Superlanguage: WideString): Boolean;
begin
  if Length(Sublanguage) = Length(Superlanguage) then begin
    Result := CompareText(Sublanguage, Superlanguage) = 0
  end else if Length(Sublanguage) > Length(Superlanguage) then begin
    Result := CompareText(copy(Sublanguage, 1, Length(Superlanguage) + 1), Superlanguage + '-') = 0
...
begin
  ...
  end else if Dest is TUtilsWideStringList then begin

    with TUtilsWideStringList(Dest) do begin
      BeginUpdate;
      try
        Clear;
        try
