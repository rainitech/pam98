...
//            TCustomOwnedNode.Adopt added.
//            TCustomOwnedObject.Adopt added.
// 2003-10-01 Release of version 1.0.1
//            Bug fixes.
...
type
  ...
  protected
    ...
    property OwnedObject[Index: Integer]: TCustomOwnedObject read GetOwnedObject;
  ...
  protected
    ...
    function Append(const NewChild: TCustomOwnedNode): TCustomOwnedNode; virtual;
    ...
    procedure RaiseException(const E: ExceptClass); virtual;
    function Remove(const OldChild: TCustomOwnedNode): TCustomOwnedNode; virtual;
...
begin
  inherited Create;
  if Assigned(AOwner) then AOwner.Attach(Self);
end;
...
begin
  ...
  if (Obj <> Self) and (Obj.GetOwner <> Self) then begin
...
begin
  if not Assigned(FOwnedObjects) then FOwnedObjects := TList.Create;
...
begin
  ...
  if FOwnedObjects.Count = 0 then begin
    FOwnedObjects.Free;
    FOwnedObjects := nil;
  end;
end;
...
begin
  if Assigned(FOwnedObjects)
    then Result := TCustomOwnedObject(FOwnedObjects[Index])
    else TList.Error(@SListIndexError, Index);
end;

function TCustomOwnedObject.GetOwnedObjectsCount: Integer;
begin
  if Assigned(FOwnedObjects)
    then Result := FOwnedObjects.Count
    else Result := 0;
end;
...
begin
  // Free owned nodes:
  if Assigned(FOwnedObjects) then begin
...
begin
  if Assigned(FParent) then FParent.InternalRemove(Self);
  InternalClear;
  FItems.free;
  inherited;
end;
...
function TCustomOwnedNode.Append(const NewChild: TCustomOwnedNode): TCustomOwnedNode;
begin
...
begin
  if FItems.Count > 0
    then Result := TCustomOwnedNode(FItems.First)
    else Result := nil;
end;
...
begin
  if FItems.Count > 0
    then Result := TCustomOwnedNode(FItems.Last)
    else Result := nil;
end;
...
begin
  ...
  if Assigned(Parent) then begin
    if Parent = Node
      then Result := True
      else Result := Parent.HasAsAncestor(Node);
...
begin
  ...
  with FItems do begin
    ...
    while I >= 0 do begin
      ...
      Node.FParent := nil; // For a better performance on larger trees: Avoids the ...
      FItems.Delete(I);    // ... call to FParent.InternalRemove(Self) in  the destructor.
      Node.Free;
      Dec(I);
      if I >= Count then
        I := Pred(Count);
...
begin
  ...
  if Assigned(RefChild) then begin
...
procedure TCustomOwnedNode.RaiseException(const E: ExceptClass);
begin
  if E = EHierarchyRequestError
    then raise E.Create(SHierarchyRequestError)
  else if E = ENoModificationAllowedError
    then raise E.Create(SNoModificationAllowedError)
  else if E = ENotAssignedError
    then raise E.Create(SNotAssignedError)
  else if E = ENotFoundError
    then raise E.Create(SNotFoundError)
  else if E = EWrongOwnerError
    then raise E.Create(SWrongOwnerError)
  else raise E.Create(E.ClassName);
end;

function TCustomOwnedNode.Remove(const OldChild: TCustomOwnedNode): TCustomOwnedNode;
begin
...
begin
  ...
  if NewChild <> OldChild then begin

    if Assigned(NewChild.Parent) then begin
