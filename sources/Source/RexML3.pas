...
type
  TReportML3 = class(TQuickRep)
    ...
    procedure ReportML3BeforePrint(Sender: TQuickRep;
...
var
  ReportML3: TReportML3;

procedure RunReportML3(MyReportParam: TMyReportParam);
...
procedure TReportML3.ShowReport(X:TMyReportParam);
var i:longint;
begin
  first_preview := true;
  ...
  if X.AktObdobie >= ZAKON_2009_01 then begin
    ...
    if (Components[i].ClassName='TQRDBText') and (Components[i].Tag=1) then
    begin
      (Components[i] as TQRDBText).Mask := '0.00';
      (Components[i] as TQRDBText).Font.Size := 7;
    end
    else if (Components[i].ClassName='TQRExpr') and (Components[i].Tag=1) then
    begin
      (Components[i] as TQRExpr).Mask := '0.00';
      (Components[i] as TQRExpr).Font.Size := 7;
    end;
  end;
  Q2_.Close;
  Q1_.Close;
...
procedure TReportML3.Q0_CalcFields(DataSet: TDataSet);
begin
...
procedure TReportML3.Q1_CalcFields(DataSet: TDataSet);
begin
...
begin
  ...
  try
  try
    ReportML3:=TReportML3.Create(Application);
    if X.DisableShape then QRDisableShape(ReportML3);
    if X.DisableLabel then QRDisableLabel(ReportML3);
    Screen.Cursor:=crDefault;
    ReportML3.ShowReport(X);
...
procedure TReportML3.Q2_CalcFields(DataSet: TDataSet);
begin
  Q2_Calc_MP.Value :=
  Copy(
    Trim(Q2_Meno.Value)+' '+
...
procedure TReportML3.ReportML3BeforePrint(Sender: TQuickRep;
  var PrintReport: Boolean);
begin
