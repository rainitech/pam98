...
procedure SaveGridToIni(grid: TDBGrid; const FileName: string; const SectionName: string {$IFDEF SMForDelphi5} = '' {$ENDIF} );
procedure LoadGridFromIni(grid: TDBGrid; const FileName: string; const SectionName: string {$IFDEF SMForDelphi5} = '' {$ENDIF} );
...
{$ENDIF}
    ...
    {$IFDEF VER_ENTERPRISE}
      ...
      if Column.Field.Dataset is TClientDataset then
        TClientDataset(Column.Field.Dataset).AddIndex(SMIndexName, AllFields, IdxOpts{$IFDEF SMForDelphi4}, DescFields {$ENDIF})
    {$ENDIF}
    {$IFDEF USE_KBMMEMTABLE}
      else
      if Column.Field.Dataset is TkbmMemTable then
...
procedure WriteTitleText(ACanvas: TCanvas; ARect: TRect; DX, DY: Integer;
  const Text: string; Alignment: TAlignment; TextEllipsis: TSMTextEllipsis; IsWordBreak: Boolean {$IFDEF SMForDelphi4} ; ARightToLeft: Boolean {$ENDIF});
const
...
//20051027     (GridStyle.Style <> gsNormal) and
  ...
  begin
    ...
    try
      ...
      if (Column.Field.DataType = ftBoolean) and Column.Field.Value then
        CheckState := cbChecked
    except
    end;
    Canvas.FillRect(Rect);
