...
type
  TReportVP2005A = class(TQuickRep)
    Q1_: TQuery;
    ...
    procedure ReportVP2005ABeforePrint(Sender: TQuickRep;
...
var
  ReportVP2005A: TReportVP2005A;

procedure RunReportVP2005A(const MyReportParam: TMyReportParam);
...
procedure TReportVP2005A.ShowReport;
var tl2na1: boolean;
begin
  first_preview := true;
...
try
  ...
  if X.Bullzip then begin
      VP_To_Email(TQuickRep(ReportVP2005A), Q1_, X);
  end
  else
  Self.Preview;
finally
...
procedure TReportVP2005A.Q1_CalcFields(DataSet: TDataSet);
...
begin
  ...
  try
  try
    ReportVP2005A:=TReportVP2005A.Create(Application);
    if X.DisableShape then QRDisableShape(ReportVP2005A);
    if X.DisableLabel then QRDisableLabel(ReportVP2005A);
    Screen.Cursor:=crDefault;
    ReportVP2005A.ShowReport;
...
procedure TReportVP2005A.ReportVP2005ABeforePrint(Sender: TQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TReportVP2005A.Q1_AfterOpen(DataSet: TDataSet);
begin
 if Q1_V_Obdobie.Value >= ZAKON_2009_01 then begin
