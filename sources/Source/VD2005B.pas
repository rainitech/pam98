...
type
  TReportVD2005B = class(TQuickRep)
    Q1_: TQuery;
    QRBand1: TQRBand;
    QRBand2: TQRBand;
    QRMemo1: TQRMemo;
    ...
    procedure ReportVD2005BNeedData(Sender: TObject;
      var MoreData: Boolean);
    procedure ReportVD2005BBeforePrint(Sender: TQuickRep;
...
var
  ReportVD2005B: TReportVD2005B;

procedure RunReportVD2005B(const MyReportParam: TMyReportParam);
procedure RunReportVD2005B_PDF(const MyReportParam: TMyReportParam);
...
procedure TReportVD2005B.ShowReport;
var rok: integer;
begin
  first_preview := true;
...
begin
  ...
  try
  try
    ReportVD2005B:=TReportVD2005B.Create(Application);
    if X.DisableShape then QRDisableShape(ReportVD2005B);
    if X.DisableLabel then QRDisableLabel(ReportVD2005B);
    Screen.Cursor:=crDefault;
    ReportVD2005B.ShowReport;
  except on E:Exception do
    MyError(SMyAppReportShowFailed+E.Message);
  end;
  finally
    Screen.Cursor:=crDefault;
    ReportVD2005B.Q0_.Close;
    ReportVD2005B.Free;
  end;
end;

function TReportVD2005B.podmienka:boolean;
begin
Result :=
...
procedure TReportVD2005B.ReportVD2005BNeedData(Sender: TObject;
  var MoreData: Boolean);
...
begin
  ...
  QRA31.Caption := Q1_Surname.Value;
  QRA32.Caption := Q1_Name.Value;
  QRA33.Caption := ulica;
  QRA34.Caption := cislo;
  QRA35.Caption := Q1_TP_city.Value;
  QRA36.Caption := Q1_L_STAT_TP.Value;
  QRA37.Caption := Q1_TP_zip.Value;
...
begin
  ...
  QRB31.Caption := Q1_Surname.Value;
  QRB32.Caption := Q1_Name.Value;
  QRB33.Caption := ulica;
  QRB34.Caption := cislo;
  QRB35.Caption := Q1_TP_city.Value;
  QRB36.Caption := Q1_L_STAT_TP.Value;
  QRB37.Caption := Q1_TP_zip.Value;
...
procedure TReportVD2005B.ReportVD2005BBeforePrint(Sender: TQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TReportVD2005B.Q0_CalcFields(DataSet: TDataSet);
var zac, kon, i: byte;
begin
...
begin with ReportVD2005B,DM do begin
  _str:=((str-1) div 4)+1;
  ...
  E_XFDF(uv+'Stat',Uppercase(NVL1(QVYSL_L_tp_stat.Value,'Slovensko')));
